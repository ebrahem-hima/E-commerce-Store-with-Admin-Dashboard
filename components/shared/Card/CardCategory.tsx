...
const CardCategory = ({ text, type }: Props) => {
  // const CardCategory = ({ icon: Icon, text, value, type }: Props) => {
  return (
    <Link
      href={`/search?query=${text}`}
      className="categoryCard snap-start group cursor-pointer flex-center flex-col gap-1 border border-[rgba(0,0,0,0.3)] rounded-sm h-32.5 hover:bg-primary hover:border-primary duration-300"
    >
      <span>
